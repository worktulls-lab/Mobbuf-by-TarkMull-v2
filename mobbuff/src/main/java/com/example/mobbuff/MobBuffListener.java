...
public final class MobBuffListener implements Listener {
    ...
    /** Всё изнашивается быстрее, особенно ночью. */
    @EventHandler
    public void onItemDamage(PlayerItemDamageEvent event) {
        if (!plugin.isBuffEnabled()) return;
        boolean night = MobBuffUtil.isDark(event.getPlayer().getLocation());
        double mult = night ? MobBuffUtil.DURABILITY_MULTIPLIER_NIGHT : MobBuffUtil.DURABILITY_MULTIPLIER_DAY;
